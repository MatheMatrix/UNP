...
| 指定虚拟路由器外部网络地址 | 支持 | 支持自动分配，只允许管理员修改，暂缺乏文档描述，本 Release 完成相关文档 |
| LBaaS | 支持七层策略和高可用 | 暂不支持高可用，支持 F5 BIGIP，暂缺乏文档描述，本 Release 完成相关文档 |
| GRE 隧道、PPTP、OpenVPN | 支持 | 暂不支持，支持使用山石云界安全网关 |
| 消息队列 | 可靠性一般 | 完成客户端高可用和关闭冗余的高可用选项，提升性能，分布式队列计划将来完成 |
...
| 性能 | 未提供正式数据和标准化测试方法 | 提供了简单的测试数据和较为标准化测试方法，提供优化建议和硬件建议，缺乏对测试项的说明和考虑以及各优化项的影响等，测试仪测试在下一个 Release 实现 |
| 网卡调优 | 开启队列绑定、UDP RSS 等网卡调优 | 除 CSP 内容外，正在调研 Flow Director 等，由于有研究称可能有乱序的问题，因此暂未提供直到问题全部解决；支持 RPS 等调优 |
...
1.1 是对 UnitedStack<sup>®</sup> UNP 1.0 的小幅更新，对 Neutron 的 Baseline 由 7.0.5 升级到了 7.1.1，由 UnitedStack 维护的版本升级到了 7.1.1.3，并添加了以下功能和说明：

 - 公网 IP 的带宽限制
 - 系统的 kernel dump 配置
 - 添加了 Package manifest 说明
 - 使配置部分的说明更加清晰
 - 默认推荐架构已有 DVR 切换到 L3 HA
 - 添加了 LBaaS 的说明


下一个版本 1.2 计划添加的内容有 LBaaS 性能测试与说明，高网络性能虚拟机测试与说明，VPNaaS 的说明，预配置网络说明和企业网互通说明。
